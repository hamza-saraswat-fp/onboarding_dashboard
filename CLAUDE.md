...
- The live onboarding DB is directly queryable READ-ONLY via the Supabase MCP (project `Onboarding_App`, ref `rqelncbqgepyardwtltc`) for investigating and validating against real data. SELECTs only, never INSERT / UPDATE / DELETE against production, ever. The MCP also exposes write tools (`apply_migration`, arbitrary `execute_sql`); do not use them.
...
- **Per-account progress** is completed modules over the account's *applicable* module count, not over the modules it has reached. Once an account has submitted (`submittedAt` set) it has been through the whole wizard, so the modules it has rows for ARE its applicable set (a conditional module like `customForms` simply never creates a row when it does not apply) and a finished account reads 100%. Before submitting, measure against the full wizard length (`totalSteps` = distinct `module_number` count) so a barely-started account does not read 100%. Helpers `wizardProgress` + `progressDenominator` in `lib/queries/account.ts`; the aggregate "Avg progress" tile already used `totalSteps`.
...
- **`salesforce_data` is a point-in-time snapshot** captured at link creation and passed through from Salesforce. It is not live (it can differ from the current Salesforce record) and has no date / account-age fields. `0` and `false` render literally; only genuinely absent keys show "Not provided". Any account-age feature (e.g. flagging links generated on pre-existing accounts) needs a date added to the blob upstream, or a proxy such as `company_id` (FieldPulse company creation order, so a lower id is an older company).
- **Wizard shape (validated against live data 2026-07).** The wizard has 6 distinct `module_number`s, but a session can hold up to 7 `wizard_module_data` rows because module 6 covers both `communications` and the conditional `customForms` (present only when `customFormsEnabled`). So an account's applicable-module count is 6 or 7 depending on customForms, and that is the only variation `progressDenominator` handles; every submitted account has all its rows complete, so it reads exactly 100%. `nue_gate` is a separate mechanism: it marks `skipped` rows in `import_jobs` (e.g. `users`, `teams`), which are backend imports, not wizard modules, and never affect the module denominator.
